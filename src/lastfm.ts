...
function authenticateLastFm(mainWindow: BrowserWindow, store: ElectronStore) : void {
...
function generateApiSignature(params: { [x: string]: string | undefined; method?: string; api_key?: string; token?: string; }): string {
...
async function scrobbleTrack(trackInfo: { author: string; title: string; }, store: ElectronStore) : Promise<void> {
    ...
    try {
        ...
        if (data.error) {
            ...
            console.log('Track scrobbled on Last.fm ${trackInfo.artist} - ${trackInfo.title}');
        }
    } catch (error) {
...
async function updateNowPlaying(trackInfo: { author: any; title: any; }, store: ElectronStore) : Promise<void> {
    ...
    try {
        ...
        console.error('Failed to update now playing');
    }
}

export { authenticateLastFm, getLastFmSession, scrobbleTrack, updateNowPlaying, trackChanged , shouldScrobble, timeStringToSeconds, generateApiSignature };
